...
export const FEATURES: Record<string, FeatureContent> = {
  ...
  'failure-simulation': {
    ...
    lead: 'Simulate an outage — a database lock, a region down, a 10× traffic spike, a payment-provider failure — and see exactly how far it spreads before it happens in production.',
    hero: 'simulation',
    benefits: [
      { title: 'Concrete blast radius', desc: 'See exactly which services go down, which stay up, and which business-critical paths break.' },
      { title: 'Find single points of failure', desc: 'See which components, if they fail, take the most of your system down with them.' },
    ...
    sections: [
      {
        ...
        body: 'Pick a scenario and a duration; Riscly propagates the failure through your dependency graph, shows the affected components and critical paths, then suggests the highest-leverage mitigation.',
        preview: 'simulation',
        bullets: ['Infra, traffic & vendor scenarios', 'Failure propagation paths', 'Mitigation impact estimates'],
      },
    ],
  },
  'ai-assistant': {
